...
{
    public PermissionCreateRequestValidator(IStringLocalizer<Messages> localizer)
    {
        ...
        RuleFor(x => x.Type)
            ...
            .WithMessage(localizer["PermissionTypeInvalid"].Value);
...
{
    public PermissionUpdateRequestValidator(IStringLocalizer<Messages> localizer)
    {
        RuleFor(x => x.Name).NotEmpty().MaximumLength(64);
        RuleFor(x => x.Type).NotEmpty().MaximumLength(32);
        RuleFor(x => x.Type)
