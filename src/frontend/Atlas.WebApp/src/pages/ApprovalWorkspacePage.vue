<template>
  <div class="approval-workspace-page">
    <div class="workspace-header">
      <h2 class="workspace-title">{{ t('approvalWorkspace.pageTitle') }}</h2>
...
const tabValues = ['pending', 'done', 'requests', 'cc'] as const;

const tabOptions = computed(() => [
  { label: t('approvalWorkspace.tabPending'), value: 'pending' },
  { label: t('approvalWorkspace.tabDone'), value: 'done' },
  { label: t('approvalWorkspace.tabRequests'), value: 'requests' },
  { label: t('approvalWorkspace.tabCc'), value: 'cc' },
]);
...
onMounted(() => {
  ...
  if (tab && (tabValues as readonly string[]).includes(tab)) {
    activeTab.value = tab;
  }
